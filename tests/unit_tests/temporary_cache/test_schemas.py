...
def test_post_schema_load_rejects_missing_value() -> None:
    schema = TemporaryCachePostSchema()
...
def test_post_schema_load_rejects_none_value() -> None:
    schema = TemporaryCachePostSchema()
    with pytest.raises(ValidationError):
        schema.load({"value": None})


def test_post_schema_load_rejects_invalid_json() -> None:
    schema = TemporaryCachePostSchema()
    with pytest.raises(ValidationError):
...
def test_put_schema_load_valid_json() -> None:
    ...
    result = schema.load({"value": '{"a": 1}'})
    assert result["value"] == '{"a": 1}'


def test_put_schema_load_rejects_missing_value() -> None:
    schema = TemporaryCachePutSchema()
    with pytest.raises(ValidationError) as exc_info:
        schema.load({})
    assert "value" in exc_info.value.messages
...
def test_put_schema_load_rejects_none_value() -> None:
    schema = TemporaryCachePutSchema()
    with pytest.raises(ValidationError):
        schema.load({"value": None})
